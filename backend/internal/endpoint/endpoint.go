...
func Register(register RegisterFunc, db *sqlx.DB, vk valkey.Client) {
	...
	register("GET /ui/v1/posts/{uuid}", post.GetByID(db, vk))
	register("DELETE /ui/v1/posts/{uuid}", post.Delete(db, vk))
	register("POST /ui/v1/posts/{uuid}/comment", post.AddComment(db, vk))
